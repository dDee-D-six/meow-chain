...
use crate::state::{CONSUMTIONS_INFO};

use cw_storage_plus::Bound;
use cw_utils::maybe_addr;

use crate::msg::{MinterResponse, QueryMsg, CheckConsumtionResponse,ConsumtionInfoResponse};
    ...
    Q: CustomMsg,
{
    ...
    fn nft_info(&self, deps: Deps, token_id: String) -> StdResult<NftInfoResponse<T>> {
        ...
        Ok(NftInfoResponse {
            ...
            consumtion_count: info.consumtion_count,
            consumtion_limit: info.consumtion_limit,
            invalid_to_consume: info.invalid_to_consume,
        })
    }
    ...
    Q: CustomMsg,
{
    ...
    pub fn query(&self, deps: Deps, env: Env, msg: QueryMsg<Q>) -> StdResult<Binary> {
        match msg {
            ...
            QueryMsg::ConsumtionInfo { token_id } => {
                to_binary(&query_consumtion_info(deps, token_id)?)
            }
            QueryMsg::CheckConsumtions {} => to_binary(&check_consumtions(deps)?),
...
pub fn query_consumtion_info(deps: Deps, _token_id: String) -> StdResult<ConsumtionInfoResponse> {
    let consumtions_info = CONSUMTIONS_INFO.load(deps.storage)?;
    let consumtion_count = consumtions_info.consumtion_count;
    let consumtion_limit = consumtions_info.consumtion_limit;
    let invalid = consumtions_info.invalid_to_consume;
    Ok(ConsumtionInfoResponse {
        invalid_to_consume: invalid,
        consumtion_count: consumtion_count,
        consumtion_limit: consumtion_limit,
    })
}

pub fn check_consumtions(deps: Deps) -> StdResult<CheckConsumtionResponse> {
    let consumtion_info = CONSUMTIONS_INFO.load(deps.storage)?;
    Ok(CheckConsumtionResponse {
        invalid_to_consume: consumtion_info.invalid_to_consume,
    })
}
